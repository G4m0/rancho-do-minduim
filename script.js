...
["jantas","Janta Individual com Picanha","Espeto de picanha, arroz, feijão tropeiro, mandioca, molho vermelho e vinagrete. Servida no prato.",44.9],
...
function renderProducts(){const root=document.getElementById("products-root");root.innerHTML=categories.map(c=>{const items=products.filter(p=>p.category===c.id);return `<section class="category-section" id="${c.id}"><div class="category-heading"><h2>${c.name}</h2><span>${items.length} opções</span></div>${c.note?`<p class="category-description">${c.note}</p>`:""}<div class="product-grid">${items.map(p=>`<article class="product-card"><img class="product-image" src="assets/products/${p.id}.webp" alt="${p.name}" loading="lazy" width="640" height="640"><h3>${p.name}</h3><p>${p.description}</p><div class="product-bottom"><strong class="product-price">${money(p.price)}</strong><button class="add-button" type="button" data-add="${p.id}">Adicionar</button></div></article>`).join("")}</div></section>`}).join("");root.addEventListener("click",e=>{const b=e.target.closest("[data-add]");if(!b)return;const p=products.find(x=>x.id===b.dataset.add);state.cart[b.dataset.add]=(state.cart[b.dataset.add]||0)+1;saveCart();if(typeof fbq==="function"&&p)fbq("track","AddToCart",{content_ids:[p.id],content_name:p.name,content_type:"product",value:p.price,currency:"BRL"});showToast()})}
...
document.getElementById("open-cart").addEventListener("click",()=>{if(typeof fbq==="function"&&totalQuantity()>0)fbq("track","InitiateCheckout",{value:totalValue(),currency:"BRL",num_items:totalQuantity(),content_ids:Object.keys(state.cart).filter(id=>state.cart[id]>0),content_type:"product"});renderCart();showScreen("cart-screen")});
