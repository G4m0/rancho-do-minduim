...
["espetos","Lombo de Porco — Só Espeto","Espetinho de lombo de porco.",15],["espetos","Lombo de Porco — Com Acompanhamento","Mandioca, molho vermelho e vinagrete.",16],
...
["espetos","Queijo Coalho — Só Espeto","Queijo coalho passado no mel.",13],["espetos","Queijo Coalho — Com Acompanhamento","Passado no mel, com mandioca, molho vermelho e vinagrete.",14.9],
["espetos","Queijo Provolone — Só Espeto","Queijo provolone passado no mel.",13],["espetos","Queijo Provolone — Com Acompanhamento","Passado no mel, com mandioca, molho vermelho e vinagrete.",14.9],
...
function renderProducts(){const root=document.getElementById("products-root");root.innerHTML=categories.map(c=>{const items=products.filter(p=>p.category===c.id);return `<section class="category-section" id="${c.id}"><div class="category-heading"><h2>${c.name}</h2><span>${items.length} opções</span></div>${c.note?`<p class="category-description">${c.note}</p>`:""}<div class="product-grid">${items.map(p=>`<article class="product-card"><h3>${p.name}</h3><p>${p.description}</p><div class="product-bottom"><strong class="product-price">${money(p.price)}</strong>${p.adultOnly?`<span class="adult-note">Venda e consumo somente para maiores de 18 anos.</span><button class="add-button" type="button" disabled>Somente presencial · 18+</button>`:`<button class="add-button" type="button" data-add="${p.id}">Adicionar</button>`}</div></article>`).join("")}</div></section>`}).join("");root.addEventListener("click",e=>{const b=e.target.closest("[data-add]");if(!b)return;state.cart[b.dataset.add]=(state.cart[b.dataset.add]||0)+1;saveCart();showToast()})}
...
function buildOrder(f){const e=f.elements;return{dateTime:new Date().toLocaleString("pt-BR"),orderNumber:createOrderNumber(),name:e.name.value.trim(),phone:e.phone.value.trim(),email:e.email.value.trim(),address:e.address.value.trim(),items:Object.entries(state.cart).map(([id,q])=>{const p=products.find(x=>x.id===id);return{id,name:p.name,quantity:q,unitPrice:p.price,subtotal:p.price*q}}),total:totalValue(),note:e.note.value.trim()}}
function buildMessage(o){const lines=o.items.map(i=>`• ${i.quantity}x ${i.name} — ${money(i.subtotal)}`).join("\n");return [`*NOVO PEDIDO ${o.orderNumber}*`,`Data: ${o.dateTime}`,"",`*Cliente:* ${o.name}`,`*Telefone:* ${o.phone}`,`*E-mail:* ${o.email}`,`*Endereço:* ${o.address}`,"","*ITENS*",lines,"",`*TOTAL: ${money(o.total)}*`,o.note?`*Observação:* ${o.note}`:"*Observação:* Nenhuma"].join("\n")}
...
document.getElementById("new-order").addEventListener("click",()=>{state.cart={};saveCart();document.getElementById("customer-form").reset();showScreen("menu-screen")});
