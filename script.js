...
function buildOrder(f){const e=f.elements,pickup=e.pickup.checked;return{dateTime:new Date().toLocaleString("pt-BR"),orderNumber:createOrderNumber(),name:e.name.value.trim(),phone:e.phone.value.trim(),pickup,address:pickup?"Retirada no balcão":e.address.value.trim(),items:Object.entries(state.cart).map(([id,q])=>{const p=products.find(x=>x.id===id);return{id,name:p.name,quantity:q,unitPrice:p.price,subtotal:p.price*q}}),total:totalValue(),note:e.note.value.trim()}}
function buildMessage(o){const lines=o.items.map(i=>`• ${i.quantity}x ${i.name} — ${money(i.subtotal)}`).join("\n");return [`*NOVO PEDIDO ${o.orderNumber}*`,`Data: ${o.dateTime}`,"",`*Cliente:* ${o.name}`,`*Telefone:* ${o.phone}`,`*Tipo:* ${o.pickup?"Retirada no balcão":"Entrega"}`,o.pickup?"":`*Endereço:* ${o.address}`,"","*ITENS*",lines,"",`*TOTAL: ${money(o.total)}*`,o.note?`*Observação:* ${o.note}`:"*Observação:* Nenhuma"].filter(Boolean).join("\n")}
